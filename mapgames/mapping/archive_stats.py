...
from . import Individual, Cell, genotype_to_actor, add_to_archive

########## Save Archive

def save_archive(archive, n_evals, archive_name, save_path, save_models = False, prefixe = "archive", suffixe = ""):
    """
    ...
    filename = f"{save_path}/{prefixe}_{archive_name}_" + str(n_evals) + f"{suffixe}.dat"
    ...
    if pyribs:
        ...
        if dataframe.shape[0] > 0:
            for line in range (dataframe.shape[0]):
                desc = [dataframe.at[line, "behavior_" + str(i)] for i in range (dim_map)]
                actor = genotype_to_actor([dataframe.at[line, "solution_" + str(i)] for i in range (dim_gen)], 
					  actor_fn())
                indiv = Individual(actor, desc, dataframe.at[line, "objective"], dataframe.at[line, "objective"])
                add_to_archive(indiv, desc, archive_stat, kdt, cell_fn)
            assert len(archive_stat) == dataframe.shape[0], "!!!ERROR!!! Not all indivs from Pyribs archive added."
        return archive_stat
    else:
        return archive
...
def evaluate_archive(archive, kdt, envs, nb_reeval = 50):
    ...
    Output:
    	- new_archive {dict} - archive with individuals reevaluated nb_reeval times 
			       and placed where they truly belong
    	- new_robust_archive {dict} - archive with individuals reevaluated nb_reeval times 
			              and placed only if they were robust in term of descriptor 
    """
    ...
    for indiv_idx in range(nb_controllers):
        evaluations_indiv = evaluations[indiv_idx*nb_reeval : (indiv_idx+1)*nb_reeval]
        estim_fitness = 0
        estim_bd = 0

        # loop through the re-evaluations to store results
        for idx, evaluation in enumerate(evaluations_indiv):
        ...
        individual = Individual(to_evaluate[indiv_idx*nb_reeval], estim_bd, estim_fitness)
        cell_fn = partial(Cell, 1) # Reeval archives use vanilla MAP-Elites grids
        added_in_new = add_to_archive(individual, individual.desc, new_archive, kdt, cell_fn, reeval=True)
        added_in_robust = add_on_robustness(individual, new_robust_archive, kdt, old_niches[indiv_idx])

    print("    -> Archive has ", nb_controllers, " controllers.")
    print("    -> New archive re-estimated has ", len(new_archive), " controllers.")
    print("    -> New archive re-estimated on robustness has ", len(new_robust_archive), " controllers.")
