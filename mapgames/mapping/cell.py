...
class Cell:
    ...
    def can_add(self, s):
        """ Return 1 if individual s will be added to cell in add(), 0 otherwise """
        return (len(self._content) < self.max_depth) or (s.fitness > self._content[self.max_depth - 1].fitness)
    ...
    def _update_attributes(self):
        ...
        if len(self._content) == 0:
            ...
            self.fitness = None
            self.centroid = None
        else: # Used in progress metrics and add_to_archive and save_archive and save_actors
...
class Deep_Grid_Cell():
	...
	# If cell not full, add at the end
        ...
        if len(self._content) > 1: self._ordered(n, s)
        self._update_attributes()
        return 1
    ...
    def select(self):
        ...
        if len(self._content) == 1: return self._content[0]

        # Normalised fitnesses of the cell
        norm_fitness = [(s.fitness - self.min_fitness) / (self.max_fitness - self.min_fitness) for s in self._content]
