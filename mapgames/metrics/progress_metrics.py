...
class ProgressMetrics:
    ...
    def __init__(self, label, save_path, file_name):
        ...
        self.file.write(
            f"n_eval,{prefixe}coverage,{prefixe}min_fitness,{prefixe}max_fitness,"
            + f"{prefixe}sum_fitness,{prefixe}mean_fitness,{prefixe}median_fitness,"
            + f"{prefixe}max_actor_id\n"
        )
    ...
    def update(self, n_evals, archive):
        ...
        if len(archive) > 0:
            self.max_actor_id = archive[
                max(archive, key=lambda desc: archive[desc].fitness)
            ].x.id
