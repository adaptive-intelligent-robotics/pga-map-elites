from . import ga_crossover as c
from . import ga_mutation as m
from . import PGVariation, BasicGAVariation

NAME_TO_VARIATION = {
    "sbx": c.SBXCrossover,
    "iso_dd": c.IsoDDCrossover,
    "polynomial_mutation": m.PolynomialMutation,
    "gaussian_mutation": m.GaussianMutation,
    "uniform_mutation": m.UniformMutation,
    "pg_variation": PGVariation
} 

class VariationScheduler:
    """
    The variation scheduler handle variations during the process, through 3 main methods:
        - initialise_variations : use the user args to create the variations and store it in the scheduler.
        - update : update the params used to determine the scheduling.
        - evolve : one of the main function of this repo! use the variations and the scheduling strategy.
    """

    def __init__(self, selection_op, proportion_evo = 0.5, 
                 stop_pg = 10000000, proportion_update = 1.0, proportion_update_period = 10000000, 
                 lr_update = 1.0, lr_update_period = 10000000, 
                 nr_of_steps_act_update = 1.0, nr_of_steps_act_update_period = 10000000, 
                 mutation_rate_update = 1.0, mutation_rate_update_period = 10000000
                ):
        """
        Initialise the scheduler.
        Input:
            - selection_op {ArchiveSelector} - selector used for crossover operations
            - proportion_evo {float} - proportion of pg and ga variation in the mutation
            - stop_pg {int} - nr evaluations to stop pg variation (GA/PG)
            - proportion_update {float} - proportion_evo decay (GA/PG)
            - proportion_update_period {float} - proportion_evo decay period (GA/PG)
            ...
            - nr_of_steps_act_update_period {float} - nr_of_steps_act decay period (PG)
            - mutation_rate_update {float} - mutation_rate decay (GA)
            - mutation_rate_update_period {float} - mutation_rate decay period (GA)
        Output: /
        """
    ...
    def initialise_variations(self,
                crossover_op = "iso_dd",
                mutation_op = None,
		pg_op = False,
                num_cpu = False,
        ...
        if crossover_op == "iso_dd":
            crossover_operator = c.IsoDDCrossover(min_gene, max_gene, iso_sigma, line_sigma)
        elif crossover_op == "sbx" and crossover_rate != 0:
            crossover_operator = c.SBXCrossover(min_gene, max_gene, crossover_rate, eta_c)
        else:
            crossover_operator = False

        if mutation_op == "polynomial_mutation" and mutation_rate != 0:
            mutation_operator = m.PolynomialMutation(min_gene, max_gene, mutation_rate, eta_m)
        elif mutation_op == "uniform_mutation" and mutation_rate != 0:
            mutation_operator = m.UniformMutation(min_gene, max_gene, mutation_rate, max_uniform)
        elif mutation_op == "gaussian_mutation" and mutation_rate != 0:
            mutation_operator = m.GaussianMutation(min_gene, max_gene, mutation_rate, sigma)
        else:
            mutation_operator = False

        if mutation_operator != False or crossover_operator != False: 
            self.ga_variation = BasicGAVariation(self.selection_op, mutation_operator, crossover_operator)
    ...
    def update(self, n_evals):
        ...
        if n_evals >= self.stop_pg and self.proportion_evo != 1.0:
            ...
            modified = True
            print(" -> Stoped PG")
        elif n_evals < self.stop_pg and self.proportion_update != 1 and \
             (n_evals - self.proportion_update_evals) >= self.proportion_update_period:
        ...
        if self.pg_variation != None:
            if self.lr_update != 1 and (n_evals - self.lr_update_evals) >= self.lr_update_period:
                self.pg_variation.update_lr(self.lr_update)
                self.lr_update_evals = n_evals
                modified = True
                print(" -> Reducing lr:", self.pg_variation.lr)
            if self.nr_of_steps_act_update != 1 and \
               (n_evals - self.nr_of_steps_act_update_evals) >= self.nr_of_steps_act_update_period:
                self.pg_variation.update_nr_of_steps_act(self.nr_of_steps_act_update)
                self.nr_of_steps_act_update_evals = n_evals
                modified = True
                print(" -> Reducing nr_of_steps_act:", self.pg_variation.nr_of_steps_act)

        # Second update the GA variation
        if self.ga_variation != None:
            if self.mutation_rate_update != 1 and \
               (n_evals - self.mutation_rate_update_evals) >= self.mutation_rate_update_period:
                modified_ga = self.ga_variation.update_mutation_rate(self.mutation_rate_update)
                self.mutation_rate_update_evals = n_evals
                if modified_ga:
                    modified = True
                    print(" -> Reducing mutation_rate:", self.ga_variation.mutation.mutation_rate)
    ...
    def evolve(self, 
        ...
        Output: 
            ...
            - controllers_variations {list} - list of variations applied to each individual
        """

        # If no variation set, return no parent
        if self.pg_variation == None and self.ga_variation == None:
            return [], []
        ...
        if self.ga_variation != None:
            ...
            controllers_variations += [self.ga_variation.label] * len(controllers_to_evolve)
        elif nb_controllers_ga != 0:
            print("\n!!!WARNING!!! No GA variation but n_evo > 0\n")

        # Second, evolve with PG
        if self.pg_variation != None:
            controllers_to_evolve = parent_controllers[nb_controllers_ga : nb_controllers_ga + nb_controllers_pg]
            offspring_controllers += self.pg_variation(controllers_to_evolve, critic, states)
            controllers_variations += [self.pg_variation.label] * len(controllers_to_evolve)
    ...
    def close(self):
        ...
        for variation in [self.ga_variation, self.pg_variation]:
            if hasattr(variation, 'close'): # close only processes that need to be closed
                variation.close()
