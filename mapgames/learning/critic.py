...
class Critic(object):
	...
	def update_greedy(self, new_actor):
		...
		Input: new_acctor {Actor} - candidate new actor
		Output: /
		"""
	...
	def train(self, replay_buffer, nr_of_steps_crit, train_batch_size=256):
		"""
        	...
        	Input:
            		- replay_buffer {ReplayBuffer} - used to sample transitions to train the critic
            		- nr_of_steps_crit {int} - number of steps to train the critic
            		- train_batch_size {int} - size of the batch sampled for each training step of the critic
		...
		for _ in  range(nr_of_steps_crit):
			...
			state, action, next_state, reward, not_done = replay_buffer.sample(train_batch_size)

            		# Conpute Q target
			all_target_Q = torch.zeros(train_batch_size, 1)
			with torch.no_grad():
