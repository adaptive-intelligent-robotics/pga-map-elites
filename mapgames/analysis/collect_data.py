    ...
    *,
    ...
    for exp in environment_names:
        ...
        if variant_names == []:
            ...
            data, n_repl = sub_collect_data(
                data, n_repl, data_path, "", exp, filename, verbose=verbose
            )
        ...
        for variant in variant_names:
            ...
            data, n_repl = sub_collect_data(
                data, n_repl, data_path, variant, exp, filename, verbose=verbose
            )
    ...
    data = data.rename(
        columns=lambda x: " ".join([s[0].upper() + s[1:] for s in x.split("_")])
    )
    ...
    if "Evaluations" in data.columns:
        data = uniformise_evaluations(data)
    ...
    for exp in data["Experiment"].drop_duplicates().values:
        ...
        if "Coverage" in data.columns:
            for algo in (
                data[data["Experiment"] == exp]["Algorithm"].drop_duplicates().values
            ):
                print(
                    "    -> Found",
                    len(
    ...
    if "Evaluations" in data.columns:
        data = data.sort_values(
            ["Algorithm", "Replication", "Evaluations"], ignore_index=True
        )
    return data
...
def remove_duplicate_columns(data):
    ...
    for column in list_of_all_columns:
        if (
            list_of_all_columns.count(column) > 1
            and column not in duplicated_columns_list
        ):
            duplicated_columns_list.append(column)
    for column in duplicated_columns_list:
        list_of_all_columns[list_of_all_columns.index(column)] = (
            column + "_1"
        )  # Rename one of the two columns as _1
    data.columns = list_of_all_columns
    for column in duplicated_columns_list:
        for line in range(data.shape[0]):
            if not (np.isnan(data.at[line, column + "_1"])) and np.isnan(
                data.at[line, column]
            ):
                data.at[line, column] = data.at[line, column + "_1"]
            elif (
                not (np.isnan(data.at[line, column + "_1"]))
                and not (np.isnan(data.at[line, column]))
                and data.at[line, column + "_1"] != data.at[line, column]
            ):
                print(
                    "!!!WARNING!!! Two different values for the same point in column",
                    column,
                )
...
def uniformise_evaluations(data):
    for exp in data["Experiment"].drop_duplicates().values:
        for variant in data["Algorithm"].drop_duplicates().values:
            sub_data = data[
                (data["Experiment"] == exp) & (data["Algorithm"] == variant)
            ]
            sub_data = sub_data.sort_values(
                ["Replication", "Evaluations"], ignore_index=True
            )
            ...
            for i, repl in enumerate(replications):
                replications_evals.append(
                    sub_data[sub_data["Replication"] == repl]["Evaluations"].values
                )
            ...
            for i, repl in enumerate(replications):
                for j in range(len(replications_evals[i])):
                    data.loc[
                        (data["Experiment"] == exp)
                        & (data["Algorithm"] == variant)
                        & (data["Replication"] == repl)
                        & (data["Evaluations"] == replications_evals[i][j]),
                        "Evaluations",
                    ] = evals[j]
    return data
...
def add_qd_score(data, exp, prefixe, min_fit):
    ...
    if min_fit is None:
        return data
    if prefixe + "Sum Fitness" not in data.columns:
        return data
    if prefixe + "Coverage" not in data.columns:
        return data
    qd_score = (
        data[data.Experiment == exp][prefixe + "Sum Fitness"]
...
def add_variation(data, stat_period, batch_size):
    ...
    """
    if (
        "Nb Evo" not in data.columns
        or "Improv Eff" not in data.columns
        or "Nb Improv" in data.columns
    ):
        return data
    ...
    if (
        "Nb Evo" not in data.columns
        and "Discovery Eff" not in data.columns
        or "Nb Discovery" in data.columns
    ):
        return data
