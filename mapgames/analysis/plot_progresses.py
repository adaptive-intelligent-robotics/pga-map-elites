from . import collect_data, first_second_third_quartile, customize_axis, adjust_box_widths
import numpy as np
...
def plot_progress(*, 
		  ...
		  stackplot = False,
                  verbose=False):
    """
    Plot values of different metrics over the time (time must be understood as number of evaluations).
    Done for each algo variant and for each environment. 
    Inputs:
        - data_path {str}
	- save_path {str}
    ...
    # Collect the datas
    verbose and print("\nCollecting data.")
    data = collect_data(data_path=data_path, 
    ...
    color_frame = pd.DataFrame(data = {'Label' : labels, 'Color' : colors[:len(labels)][:]})

    # Create replicaiton frame
    replication_frame = pd.DataFrame(columns = {"Experiment", "Algorithm", "Max Evaluations", "Replications"})
    loss_frame = pd.DataFrame(columns = {"Experiment", "Algorithm", "Max Evaluations", "Stat", "Prop Loss"})

    # Print all graphs
    for idx, exp in enumerate(environment_names):
        exp_data = data[data["Experiment"]==exp]
        exp_labels = exp_data[hue].drop_duplicates().values
        if len(exp_labels) == 0: continue
        ...
        for stat in exp_data.columns:
            ...
            if stat == hue or stat == "Evaluations" or stat == "Algorithm" or \
               stat == "Experiment" or stat == "Replication": continue
            if exp_data[stat].empty: continue
            if exp_data[stat].isnull().all(): continue
            if exp_data[stat].dtype == list: continue

	        # Select dataframe
            if hue != "Algorithm": columns = ["Algorithm", "Replication", "Evaluations", hue, stat]
            else: columns = ["Algorithm", "Replication", "Evaluations", stat]
            stat_data = exp_data[exp_data[stat] == exp_data[stat]][columns] # Remove NaN + keep only columns


            # Fill in replication frame
            replication_frame = fill_in_replication_frame(replication_frame, stat_data, exp)

            # Fill in loss frame
            loss_frame = fill_in_loss_frame(loss_frame, name_exp, exp_data, stat, stat_data)
            ...
            try:
                ...
                if stackplot:
                    ...
                    # Plot
                    fig = plt.figure()
                    ax = sns.lineplot(x="Evaluations", y=stat, data=values, hue = hue, style = hue)
                ...
                # Lineplot
                    ...
                    if all([a == "1.00" or a == "0.00" or a == "0.25" or a == "0.50" or a == "0.75" \
                    ...
                    ax = sns.lineplot(x="Evaluations", y=stat, data=stat_data, hue = hue, 
                                      estimator = np.median, ci=None, style = hue)
                # Customise axis
                customize_axis(ax)
                ...
                # Save figure
                fig.savefig(os.path.join(save_path, f"{filename}_{name_exp}_{name_stat}_{case_name}.svg"), \
                            bbox_inches='tight')
                plt.close()
            ...
            try:
                if p_values: 
                    ...
                    compute_p_values(stat_data, hue, save_path, stat, name_exp, name_stat, \
                                     case_name, filename=filename)
            except:
    ...
    replication_frame.to_csv(os.path.join(save_path, f"{filename}_replications_{case_name}.csv"), \
                             header=None, index=None, sep=',', mode='a')

    # Plot reevaluation loss
    loss_plot(loss_frame, save_path, filename, case_name, color_frame, p_values=p_values)
    loss_frame.to_csv(os.path.join(save_path, f"{filename}_losss_{case_name}.csv"), \
                      header=None, index=None, sep=',', mode='a')
    return data
...
def variation_usage_prints(hue, exp, stat, stat_data):
    ...
    for label in sub_stat_data[hue].drop_duplicates().values:
        ...
        for label in sub_stat_data[hue].drop_duplicates().values:
            if line_idx > 0:
                frame = stat_data[(stat_data["Evaluations"] < min_eval_0) & (stat_data[hue] == label)]
                frequency = frame[stat].sum() / frame.shape[0]
                print("        for", label, "in average", frequency, \
...
def fill_in_replication_frame(replication_frame, stat_data, exp):
    ...
    for algo in stat_data["Algorithm"].drop_duplicates().values:
        if not(replication_frame.empty) and \
	   not(replication_frame[(replication_frame["Experiment"] == exp) & \
		                 (replication_frame["Algorithm"] == algo)].empty): 
               continue
        max_eval = min_number(stat_data, "Algorithm", "Evaluations", algo)
        replication = len(stat_data[(stat_data["Algorithm"] == algo) & \
			            (stat_data["Evaluations"] == max_eval)]\
                                    ["Replication"].drop_duplicates().values)
        replication_frame = replication_frame.append({"Experiment": exp,
						      "Algorithm": algo, 
...
def fill_in_loss_frame(loss_frame, exp, exp_data, stat, stat_data):
    ...
    if not("Reeval" in stat): return loss_frame

    # Find name of the original stat
    non_reeval_stat = ""
    non_reeval_stat_name = stat[7:]
    for other_stat in exp_data.columns:
        if non_reeval_stat_name in other_stat and not("Robust" in other_stat) and not(other_stat == stat):
            if non_reeval_stat != "":
                print("!!!WARNING!!! two stats correspond to non-reeval version of", stat, \
                      ":", other_stat, "and", non_reeval_stat)
            else:
    ...
    non_reeval_stat_data = exp_data[exp_data[non_reeval_stat] == exp_data[non_reeval_stat]]
    for algo in stat_data["Algorithm"].drop_duplicates().values:

        # Add the loss for each replication to the frame
        stat_value = []
        for replication in stat_data[stat_data["Algorithm"] == algo]["Replication"].drop_duplicates().values:
            frame = stat_data[(stat_data["Algorithm"] == algo) & (stat_data["Replication"] == replication)]
        ...
        for replication in non_reeval_stat_data[non_reeval_stat_data["Algorithm"] == algo]\
                           ["Replication"].drop_duplicates().values:
            non_reeval_frame = non_reeval_stat_data[(non_reeval_stat_data["Algorithm"] == algo) & \
                                                    (non_reeval_stat_data["Replication"] == replication)]
            max_eval = max_number(non_reeval_frame, "Algorithm", "Evaluations", algo)
            non_reeval_stat_value.append(max(non_reeval_frame[non_reeval_frame["Evaluations"] == max_eval]\
                                         [non_reeval_stat].values))

        for i in range (len(stat_value)):
            if non_reeval_stat_value[i] == 0:
                if stat_value[i] == 0: loss = 0
                else: loss = 100
            else:
                loss = (non_reeval_stat_value[i] - stat_value[i]) / non_reeval_stat_value[i] * 100
            if loss != loss or (loss < 0 and ("Coverage" in stat or "Score" in stat)):
                print("!!!!WARNING!!!! Incorrect loss value:", loss, "for", stat, "on", exp, "and", algo)
            loss_frame = loss_frame.append({"Experiment": exp,
                                            "Algorithm": algo,
                                            "Max Evaluations": max_eval,
                                            "Stat": stat,
                                            "Prop Loss": loss},
...
def loss_plot(loss_frame, save_path, filename, case_name, color_frame, p_values):
    ...
    for stat in loss_frame["Stat"].drop_duplicates().values:
        ...
        try:
            ...
            ax = sns.boxplot(x="Experiment", y="Prop Loss", hue="Algorithm", data=sub_frame, width=0.6)
            customize_axis(ax)
            ...
            name_stat = stat.replace(" ", "_")
            fig.savefig(os.path.join(save_path, f"{filename}_loss_boxplot_{name_stat}_{case_name}.svg"), \
                        bbox_inches='tight')
            plt.close()
            ...
            if p_values: 
                for exp in sub_frame["Experiment"].drop_duplicates().values:
                    ...
                    print(sub_sub_frame)
                    try:
                        compute_p_values(sub_sub_frame, "Algorithm", save_path, "Prop Loss", name_exp, name_stat, \
                                         case_name, filename = filename + "_loss")
                    except:
                        print("\n!!!WARNING!!! Error when computting p-values for loss in", stat, "in", exp)
                        print(traceback.format_exc(-1))
    ...
    for i_stack in range(len(labels)):
        ...
        if i_stack == 0 or values.empty:
            for stack_x in x:
                ...
                values = values.append({'Evaluations': stack_x, \
				       stat: value, \
				       hue: labels[i_stack]}, \
				       ignore_index=True)
        else:
            for stack_x in x:
                value = values[(values["Evaluations"] == stack_x) & \
                               (values[hue] == labels[i_stack - 1])][stat].values[0] + \
...
def stack_plot_error(frame, hue, stat, ax, exp_palette):
    ...
    ax.fill_between(x = x, 
                    ...
                    color = exp_palette[0], alpha = 0.3)
    for i_stack in range(1, len(labels)):
        ax.fill_between(x = x, 
                        y1 = np.array(frame[frame[hue] == labels[i_stack-1]][stat].values).astype(np.float), \
                        y2 = np.array(frame[frame[hue] == labels[i_stack]][stat].values).astype(np.float), \
                        color = exp_palette[i_stack], alpha = 0.3)
...
def p_value_ranksum(frame, hue, reference_label, compare_label, stat):
    _, p = ranksums( 
                     get(get(frame, hue, reference_label), \
                     "Evaluations", max_number(frame, hue, "Evaluations", reference_label))[stat].to_numpy(), \
	             get(get(frame, hue, compare_label), \
		     "Evaluations", max_number(frame, hue, "Evaluations", compare_label))[stat].to_numpy()
		   )
    return p


def compute_p_values(frame, hue, save_path, stat, name_exp, name_stat, case_name, filename="progress"):
    ...
    for reference_label in labels:
        for compare_label in labels:
            p_frame = p_frame.append({'Reference label' : reference_label,
	    			      'Label' : compare_label,
				      'p-value' : p_value_ranksum(frame, hue, reference_label, compare_label, stat)
				     }, ignore_index=True)
    p_frame = p_frame.pivot(index='Reference label', columns='Label', values='p-value')
    p_file = open(os.path.join(save_path, f"{filename}_pvalue_{name_exp}_{name_stat}_{case_name}.md"), "a")
