...
def get_files(*, data_path, variant, env, evals="", filetype="*.csv", prefixe="", verbose=False):
    """
    Return all files name with the env and variant names in it, and the good filetype (ie extension). 
    """
    return [os.path.join(root, name) for root, dirs, files in os.walk(data_path) \
                                     for name in files \
				     if (variant in name and env in name and evals in name 
				         and name.endswith(filetype) and prefixe in name)]

################## Find min or max of dataframe

def find_min_max_prefixe(data, exp, prefixe, use_min = True):
    name = "Min" if use_min else "Max"
    if (prefixe + name + " Fitness") not in data.columns or \
       data[data['Experiment'] == exp][prefixe + name + ' Fitness'].isnull().values.all():
        return None
    if use_min:
...
def find_min_max(data, exp, use_min = True):
    ...
    extr_fit = extr_fit if extr_reeval_fit == None else (extr_reeval_fit if extr_fit == None else \
               (min(extr_fit, extr_reeval_fit) if use_min else max(extr_fit, extr_reeval_fit)))
    extr_robust_fit = find_min_max_prefixe(data, exp, "Robust ")
    extr_fit = extr_fit if extr_robust_fit == None else (extr_robust_fit if extr_fit == None else \
               (min(extr_fit, extr_robust_fit) if use_min else max(extr_fit, extr_robust_fit)))
    if extr_fit != None:
...
def find_n_niches(env_name):
    if env_name in ["QDHalfCheetahBulletEnv-v0", "QDWalker2DBulletEnv-v0", \
...
def find_cell_files(filename, variant):
    ...
    if "reeval_" in filename:
        if "MAP-Elites-ES" in variant:
            return [path + "archive_reeval/cell_ids.pk", path + "archive_reeval/cell_boundaries.pk"]
        return [path + "reeval_cell_ids.pk", path + "reeval_cell_boundaries.pk"]
    if "robust_" in filename:
        if "MAP-Elites-ES" in variant:
            return [path + "archive_robust/cell_ids.pk", path + "archive_robust/cell_boundaries.pk"]
